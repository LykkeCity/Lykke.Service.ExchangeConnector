...
{
    public sealed class JfdTradeSessionConnector : IApplication, IDisposable
    {
        ...
        public JfdTradeSessionConnector(JfdConnectorConfiguration config, ILog log)
        {
            _config = config;
        ...
        public void Start()
        {
            _log.WriteInfoAsync(nameof(Start), string.Join("/n", _config.FixConfig), "Starting fix connection with configuration").Wait();
